...
--
-- 크로스도메인 FK 보류: lifedex_item_id → LIFEDEX_ITEMS(팀원3)는 해당 테이블이 아직 없다.
-- user_id → USERS(팀원1)는 테이블이 V2__auth_and_users에 있지만, 타 담당 테이블에 제약을
-- 발생시키므로 퀘스트 도메인 담당(팀원2)이 단독으로 결정할 사안이 아니다. 둘 다 FK 없이 BIGINT
