...
public class GroupMembershipService {
    ...
    @Transactional public void remove(Long groupId,Long ownerId,Long targetId){Group group=groupService.lockedOwner(groupId,ownerId);groupService.requireActive(group);if(ownerId.equals(targetId))throw new BusinessException(ErrorCode.OWNER_CANNOT_LEAVE);GroupMember m=requireMember(groupId,targetId);m.remove(now());}
