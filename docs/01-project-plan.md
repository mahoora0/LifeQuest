...
| 레벨업 보상·칭호·대표 배지 | 팀원 1 | 레벨업 시 보상 지급, 칭호 선택 |
...
위치 인증이 필요 없는 행동형 퀘스트(독서, 전화 등)는 사용자의 직접 완료 처리로 진행한다. 상세: `05-business-rules.md` §5.
...
실시간 이동 경로 추적, 정교한 위치 조작 탐지 등 고급 기능은 Phase 2로 분리한다. 상세 규칙: `05-business-rules.md` §4.
