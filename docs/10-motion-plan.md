...
홈의 고정 섹션 **첫 6개까지만** fade + y 10px, 40ms 간격. 패키지 없이 index 기반
`TweenAnimationBuilder`. **재빌드마다 재생되면 안 된다** — 스크롤·상태 갱신 때 다시 날아오면 최악이다.

**긴 목록에는 쓰지 않는다.** `ListView`가 항목을 재활용하므로 스크롤해서 되돌아올 때마다 다시
날아온다. 이 제약 때문에 퀘스트 목록·친구 목록에는 적용하지 않았다.

| 케이스 | 방법 |
|---|---|
| 총 시간 상한 | 마지막 delay + duration ≤ **400ms**를 상수 테스트로 고정. 계획 단계의 300ms는 실측에서 틀린 값이었다 — 6항목이면 지연만 200ms라 항목 등장을 `quick`(160ms)로 줄여도 360ms다. 300ms를 지키려면 시간차를 주는 항목이 4개로 줄어 홈의 마지막 섹션이 첫 섹션과 동시에 뜬다 |
